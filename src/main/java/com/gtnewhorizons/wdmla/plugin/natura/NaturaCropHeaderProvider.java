...
public enum NaturaCropHeaderProvider implements IBlockComponentProvider {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor) {
        if (accessor.getBlock() instanceof CropBlock cropBlock) {
            ThemeHelper.INSTANCE.overrideTooltipHeader(tooltip,
                    new ItemStack(cropBlock.getCropItem(accessor.getMetadata()), 1, cropBlock.damageDropped(accessor.getMetadata())));
        }
    }
